...
// ─── DOM refs (assumed to exist in your HTML) ────────────────────────────────
...
const prioritySlider   = document.querySelector('.priority-slider');   // FIX: was used as a function
...
function updatePrioritySlider() {
    // FIX: was '..' (double dot), querySelector selector was wrong
    const selectedOption = document.querySelector('.priority-option.selected');
    if (selectedOption) {
        const optionRect    = selectedOption.getBoundingClientRect();   // FIX: getBoundingsClientRect → getBoundingClientRect
        const containerRect = prioritySelector.getBoundingClientRect(); // FIX: same typo

        const left = optionRect.left - containerRect.left - 3;
        // FIX: was calling the function itself instead of the slider element
        prioritySlider.style.transform = `translateX(${left}px)`;      // FIX: $() → ${}  and missing closing )
        ...
        switch (currentPriority) {
            case 'high':
                ...
                break;
            case 'medium':
                color = 'var(--medium-priority)';   // FIX: missing opening (
                break;
            case 'low':
                color = 'var(--low-priority)';      // FIX: missing ( and closing )
                break;
        }
...
function updateStats() {
    ...
    totalTaskEl.textContent = `${totalTasks} ${totalTasks === 1 ? 'task' : 'tasks'}`;
